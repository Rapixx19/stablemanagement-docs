...
9. **Operations calendar** at `/owner/ops` — the internal counterpart to `/owner/schedule`. **Never visible to clients** (route guard + RLS). **Desktop-only carve-out (DES-LOCK-3, 2026-05-14):** renders at `md` (≥768px) and above only. At `sm` (375–767px), shows a polite redirect: "Schichtplanung am Schreibtisch — bitte auf einem grösseren Bildschirm öffnen." plus a deep-link to the dashboard's worker-status mini-tile for at-a-glance check. The grid (5 workers × 7 days × shift bars × task pills) is functionally impossible at 375px; carving out is cheaper + better UX than scrolling. Renders:
